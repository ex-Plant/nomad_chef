...
The `feat/ebook-sales` branch bundles unrelated work (hero/nav/services polish, test cleanup). For ebook-sales review **focus only on the files listed below**. Everything else in the diff is incidental and can be reviewed/skipped independently.

### Collections (Payload schema)

- `src/collections/customers.ts` — Customers collection (email-deduped, optional addresses)
- `src/collections/products.ts` — Products collection (digital/physical conditional fields)
- `src/collections/orders/index.ts` — Orders collection (single collection w/ format-aware conditional fields)
- `src/collections/orders/hooks/generate-order-number.ts` — `NC-{YYYY}-{####}` numbering on create
- `src/collections/orders/hooks/snapshot.ts` — snapshots unitPriceGross / vatRate / vatAmount / shippingAddress at sale time
- `src/collections/orders/hooks/upsert-customer.ts` — upserts Customer by email when an Order is created via webhook
- `src/collections/orders/hooks/digital-fulfillment.ts` — on `paymentStatus → paid`, generates download token + sends digital email
- `src/collections/orders/hooks/physical-shipped.ts` — on `fulfillmentStatus → shipped`, sends tracking email
- `src/collections/users.ts` — minor admin-auth tweaks tied to Payload setup

### Domain logic

- `src/lib/billing.ts` — VAT split, order-number formatter, download-token generator (integer grosze, no floats)
- `src/lib/billing.test.ts` — node:test coverage for the helpers above
- `src/lib/email.ts` — nodemailer transport wrapper
- `src/lib/contact-schema.ts` — zod schema shared by contact form + email lib

### Routes

- `src/app/download/[token]/route.ts` — token-gated digital download (expiry + download count limit)
- `src/app/api/test-email/route.ts` — admin-only email transport smoke test

### Contact form (uses the same email transport)

- `src/components/sections/contact/contact-form.tsx`
- `src/components/sections/contact/contact-success-dialog.tsx`
- `src/components/sections/contact/contact.tsx`
- `src/components/sections/contact/field-shell.tsx`
- `src/components/ui/dialog.tsx` — shadcn dialog primitive (new)
- `src/hooks/use-scroll-lock.ts` — body scroll lock used by the dialog

### Migrations (auto-generated by Payload)

- `src/migrations/20260429_110034.{ts,json}` — Customers + Products tables
- `src/migrations/20260429_112256.{ts,json}` — Orders table + relations
- `src/migrations/20260429_112839.{ts,json}` — Orders conditional fields + indexes
- `src/migrations/20260429_113419.{ts,json}` — order numbering + download token columns
- `src/migrations/20260429_120000_drop_hero_media.ts` — unrelated cleanup (skip for ebook review)
- `src/migrations/20260429_135751.{ts,json}` — final adjustments (vat snapshot, fulfillment status)
- `src/migrations/index.ts` — registers the new migrations

### Config & infra

- `src/payload.config.ts` — wires Customers/Products/Orders + email transport
- `src/payload-types.ts` — auto-generated from collections (do not hand-edit)
- `src/config/env.ts` — adds required `EMAIL_HOST` / `EMAIL_USER` / `EMAIL_PASS` / `EMAIL_TO`
- `.env.example` — documents the new email vars
- `package.json` + `package-lock.json` — adds nodemailer + types

### Admin shell (forced by adding new collections)

- `src/app/(payload)/admin/[[...segments]]/page.tsx` — restored canonical page (AdminViewProps fix)
- `src/app/(payload)/admin/importMap.js` — Payload-generated, do not hand-edit

### NOT part of ebook-sales (skip during this review)

- `public/videos/*`, `src/components/sections/hero/*`, `src/components/sections/nav/*`, `src/components/sections/services/services-sticky.tsx`
- `src/components/shared/{loader,logo,animation-toggle-pot}*`, `src/components/ui/{animation-hint,gallery-lightbox}.tsx`
- `src/app/(site)/{layout,page}.tsx` (route-group reorg only — file moves)
- `src/config/section-ids.ts`, `src/globals/site.ts`, `src/lib/get-site.ts`
- `src/scripts/seed-content.ts`, `src/app/tests/page.tsx`, `tests/*` (deletions)

### Vercel environment variables (pushed 2026-04-29)

`EMAIL_HOST`, `EMAIL_USER`, `EMAIL_PASS`, `EMAIL_TO` set across Production / Preview / Development. Verified via `vercel env ls`.
